 ...
 */
package hr.fer.rassus.homework2.utils;
...
public class SimpleSimulatedDatagramSocket extends DatagramSocket {
	...
	public SimpleSimulatedDatagramSocket(final int port, final double lossRate, final int averageDelay)
	...
	public SimpleSimulatedDatagramSocket(final double lossRate, final int averageDelay)
			throws SocketException, IllegalArgumentException {
		random = new Random();
	...
	private class OutgoingDatagramPacket implements Runnable {
		...
		private OutgoingDatagramPacket(final DatagramPacket packet, final long time) {
			this.packet = packet;
			this.time = time;
		}
